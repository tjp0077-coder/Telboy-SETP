...
import { api, MessageItem } from "@/src/api";
  ...
  return (
    <View style={[styles.screen, { paddingTop: insets.top }]} testID="messages-screen">
      <View style={styles.header}>
        ...
        <Text style={styles.headerSub}>Live announcements from the symposium team</Text>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              <Ionicons name="megaphone-outline" size={48} color={colors.onSurfaceMuted} />
              <Text style={styles.emptyText}>No announcements yet</Text>
            </View>
          }
          renderItem={({ item }) => {
            const p = PRIORITY_STYLE[item.priority] || PRIORITY_STYLE.info;
            const when = formatDate(item.created_at);
            return (
              <View style={[styles.msg, shadow.card]} testID={`message-${item.id}`}>
...
const styles = StyleSheet.create({
  ...
  msgAuthor: { fontSize: 12, color: colors.onSurfaceMuted, flex: 1 },
  delBtn: { padding: 4 },
